...
export function* getQuestion(action) {
  try {
    ...
    const data = yield call(sender, {
      url: `${BASE_URL}/questionlist?${serialize(lastQuery)}`,
      method: 'GET',
    });
    if (data) {
...
export function* deleteQuestion(action) {
  try {
    const data = yield call(sender, {
      url: `${BASE_URL}/questionlist/${action.actions}`,
      method: 'DELETE',
    });
    if (data) {
...
export function* editQuestion(action) {
  try {
    const data = yield call(sender, {
      url: `${BASE_URL}/questionlist/${action.actions._id}`,
      method: 'PUT',
      data: action.actions,
    });
    if (data) {
...
export default function* questionlistListPageSaga() {
